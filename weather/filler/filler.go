...
func main() {
	...
	// write to script
	wf, err := os.Create(conf.Script)
	defer wf.Close()
	if err != nil {
		os.Exit(1) // nolint:gocritic
	}
	wf.WriteString("#!/bin/bash\nset -e\n\npsql -v ON_ERROR_STOP=1 -U postgres -d weather <<-EOSQL\n" +
		"insert into cities(name, lat, lon, country, state) values\n")
	for i, v := range citiesToDB {
		v.Name = strings.ReplaceAll(v.Name, "'", "")
		v.Country = strings.ReplaceAll(v.Country, "'", "")
		v.State = strings.ReplaceAll(v.State, "'", "")
		if i < len(citiesToDB)-1 {
			wf.WriteString("('" + v.Name + "', " + fmt.Sprint(v.Lat) + ", " + fmt.Sprint(v.Lon) + ", '" + v.Country + "', '" + v.State + "'),\n")
		} else {
			wf.WriteString("('" + v.Name + "', " + fmt.Sprint(v.Lat) + ", " + fmt.Sprint(v.Lon) + ", '" + v.Country + "', '" + v.State + "');\n")
		}
	}
	wf.WriteString("EOSQL")
}
